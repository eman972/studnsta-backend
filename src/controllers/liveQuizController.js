...
exports.createLiveQuiz = async (req, res) => {
  try {
    ...
    // Generate a unique placeholder link or ID for the quizLink field
    const uniqueLinkId = Math.random().toString(36).substring(2, 10);
    ...
    const quiz = await Quiz.create({
      ...
      quizLink: uniqueLinkId, // Set unique quizLink to avoid E11000 duplicate key error
      liveSettings: {
        allowRetake: false,
        showResults: true,
    ...
    // Generate unique quiz link for response
...
exports.getLiveQuizById = async (req, res) => {
  try {
    ...
    const quiz = await Quiz.findById(id)
      ...
      .populate("questions", "-correctAnswer -explanation"); // Populate questions and hide answers
    ...
    // Feature 67: per-student shuffle
    ...
    if (quiz.liveSettings?.randomizeQuestions) {
      questions = questions.sort(() => Math.random() - 0.5);
    }
    if (quiz.liveSettings?.randomizeOptions) {
